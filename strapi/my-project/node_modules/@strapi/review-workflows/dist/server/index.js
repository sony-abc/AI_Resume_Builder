...
const workflows$1 = ({ strapi: strapi2 }) => {
  ...
  return {
    ...
    assertStageBelongsToWorkflow(stageId, workflow2) {
      ...
      if (!belongs) {
        throw new utils.errors.ApplicationError(`Stage does not belong to workflow "${workflow2.name}"`);
      }
    }
  };
};
...
const stageObject = utils.yup.object().shape({
  ...
  // hex color
  permissions: utils.yup.array().of(
    utils.yup.object().shape({
      role: utils.yup.number().integer().min(1).required(),
      action: utils.yup.string().oneOf([STAGE_TRANSITION_UID]).required(),
...
const validateContentTypes = utils.yup.array().of(
  utils.yup.string().test({
...
const validateUpdateAssigneeOnEntity = utils.validateYupSchema(
...
const workflows = {
  ...
  async find(ctx) {
    ...
    ctx.body = {
      data: await utils.async.map(workflows2, sanitizeOutput),
      meta: {
        workflowCount
      }
    };
...
const stages = {
  ...
  async find(ctx) {
    ...
    ctx.body = {
      data: await utils.async.map(stages2, sanitizer)
    };
  },
  /**
   * Get one stage
